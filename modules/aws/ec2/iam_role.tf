...
resource "aws_iam_role" "vmdk-role" {
  name = "aws_vmdk_role"
  assume_role_policy = "${file("role-vmimport.tpl")}"
...
resource "aws_iam_policy" "vmdk_policy" {
  ...
  policy = "${file("policy-vmimport.tpl")}"
}

####policy attachment####

resource "aws_iam_policy_attachment" "vmdk-policy-attachment" {
  name       = "vmdk-policy-attachment"
  roles      = [aws_iam_role.vmdk-role.name]
